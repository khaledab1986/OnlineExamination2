...
using OnlineExamination.Views.Admin;
                    ...
                    try
                    {
                     ...
                     //   string json = App.con.GetStringAsync(tt).Result;
                       // var tr = JsonConvert.DeserializeObject<IList<UserClass>>(json);
                        ...
                        if (User_kind == 2) { await Shell.Current.Navigation.PushAsync(new AdminPage()); }
                        if (User_kind == 1) {await Shell.Current.Navigation.PushAsync(new Course()); }
                        if (User_kind == 0) { await Shell.Current.Navigation.PushAsync(new ExamView_s()); }
                    }
                    catch (Exception ee)
                    {
